...
function IconMap() {
  return (
    <svg viewBox="0 0 24 24" className="w-7 h-7 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
...
export function MobileNav() {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-50 md:hidden border-t-[3px] border-fta-black bg-fta-paper pb-6"
        ...
        {TABS.map(({ href, label, Icon }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className="flex flex-col items-center justify-center gap-1 min-w-0 py-2.5 border-0 border-fta-black font-bold text-sm uppercase transition-colors touch-manipulation"
